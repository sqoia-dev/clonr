...
func RegisterSelectorFlags(cmd *cobra.Command, set *SelectorSet) {
	...
	cmd.Flags().StringVar(&set.Chassis, "chassis", "", "Chassis names (comma-separated) — enclosure labels from the datacenter inventory")
...
type SelectorDB interface {
	...
	// ListNodeIDsByEnclosureLabels returns node IDs for all nodes placed in
	// enclosures whose label matches any of the given values.
	// Empty slice (not error) when no nodes are assigned to matching enclosures.
	ListNodeIDsByEnclosureLabels(ctx context.Context, labels []string) ([]NodeID, error)
}
...
func Resolve(ctx context.Context, db SelectorDB, set SelectorSet) ([]NodeID, error) {
	...
	// --chassis — resolves to all node IDs in enclosures whose label matches
	// any of the comma-separated chassis names (#231).
	if set.Chassis != "" {
		names := strings.Split(set.Chassis, ",")
		for i, n := range names {
			names[i] = strings.TrimSpace(n)
		}
		chassisIDs, chassisErr := db.ListNodeIDsByEnclosureLabels(ctx, names)
		if chassisErr != nil {
			return nil, fmt.Errorf("selector: chassis %q: %w", set.Chassis, chassisErr)
		}

...
//
// GAP-104a-2: includes updated ldap_default_authtok so bind-password drift is corrected.
// GAP-104a-4: includes "ssh" in services and ldap_user_ssh_public_key so LDAP pubkey auth
//             is active on the pushed config without requiring a full reimage.
// GAP-104a-1: ldap_tls_reqcert = allow (matches deploy path) so nodes accept the new CA
//             after update-ca-trust runs; "demand" caused verify error:num=19 on rotation.
...
// PushLDAPOnNodeConnect is called by the clientd hello handler whenever a node
// (re)connects. It pushes the current LDAP CA cert, sssd.conf, and sshd drop-in
// to the node if and only if:
//  1. The LDAP module is enabled and ready.
//  2. The node has an ldap_node_state row (was configured during a previous deploy).
//
// This closes the reconnect gap: FanoutLDAPConfig fires immediately after Enable()
// but nodes that are offline at that moment are recorded as Offline=true and log
// "will receive config on reconnect" — this function delivers that promise.
//
// Non-blocking guarantee: the push runs synchronously in the hello handler's
// goroutine. The 30-second-per-push ack timeout in sendConfigPush bounds the
// latency added to the hello path. If the push fails, the error is logged and
// the hello handler continues normally — a failed reconnect push is non-fatal;
// the node may have a stale CA until the next successful push or reimage.
func (m *Manager) PushLDAPOnNodeConnect(ctx context.Context, nodeID string) {
	m.mu.RLock()
	hub := m.hub
	m.mu.RUnlock()
	if hub == nil {
		return // dev/test mode — no hub wired
	}

	row, err := m.db.LDAPGetConfig(ctx)
	if err != nil || !row.Enabled || row.Status != statusReady {
		return // LDAP not active — nothing to push
	}

	configured, err := m.db.LDAPNodeIsConfigured(ctx, nodeID)
	if err != nil {
		log.Warn().Err(err).Str("node_id", nodeID).
			Msg("ldap reconnect push: LDAPNodeIsConfigured failed — skipping push")
		return
	}
	if !configured {
		return // node was never LDAP-configured — nothing to push
	}
	...
	// Verify the node is still connected before pushing. In production this is
	// always true when called from handleHello (the WS just upgraded), but the
	// guard prevents a redundant push if the connection dropped between the hello
	// dispatch and here, and makes the function safe to call from other contexts.
	if !hub.IsConnected(nodeID) {
		log.Debug().Str("node_id", nodeID).
			Msg("ldap reconnect push: node no longer connected — skipping push")
		return
	}

	log.Info().Str("node_id", nodeID).Msg("ldap reconnect push: node reconnected while enrolled — pushing current config")
	res := m.pushLDAPToNode(ctx, hub, nodeID, row, row.CACertPEM, svcPasswd, serverURI)
	if res.OK {
		log.Info().Str("node_id", nodeID).Msg("ldap reconnect push: config applied successfully")
	} else {
		log.Warn().Str("node_id", nodeID).Str("error", res.Error).
			Msg("ldap reconnect push: config apply failed — node may have stale LDAP config until next reimage")
	}
}

// sshdLDAPKeyDropin is the content of the sshd_config.d drop-in that wires
// AuthorizedKeysCommand to sss_ssh_authorizedkeys (GAP-104a-4).
// Matches the drop-in written by deploy/finalize.go:writeLDAPConfig.
const sshdLDAPKeyDropin = "# 50-clustr-ldap-keys.conf — generated by clustr-serverd\n" +
	"# Enables SSH public-key authentication via LDAP (sshPublicKey attribute).\n" +
	"# DO NOT EDIT — managed by clustr.\n" +
	"AuthorizedKeysCommand /usr/bin/sss_ssh_authorizedkeys\n" +
	"AuthorizedKeysCommandUser nobody\n"

// pushLDAPToNode sends the CA cert, sssd.conf, and sshd AuthorizedKeysCommand
// drop-in to a single node sequentially. Returns a NodePushResult summarising the outcome.
//
// Push order:
//  1. "ldap-ca-cert"   — /etc/pki/ca-trust/source/anchors/clustr-ca.crt
//     Follow-up: update-ca-trust extract + systemctl restart sssd.
//  2. "sssd"           — /etc/sssd/sssd.conf (with new bind password + ssh service).
//     Follow-up: systemctl restart sssd.
//  3. "sshd-ldap-keys" — /etc/ssh/sshd_config.d/50-clustr-ldap-keys.conf (GAP-104a-4).
//     Follow-up: systemctl reload sshd (graceful; active sessions preserved).
//
// Per-push audit records are written with actions ldap.ca_pushed,
// ldap.bind_password_pushed, and ldap.sshd_keys_pushed.
func (m *Manager) pushLDAPToNode(
	...
	log.Info().Str("node_id", nodeID).Msg("ldap fanout: ca-cert applied")
	m.audit.Record(ctx, "", "clustr-serverd", db.AuditActionLDAPCAApplied,
		"node", nodeID, "",
		nil,
		map[string]string{"ca_fingerprint": row.CACertFingerprint},
	)

	// 2. Push sssd.conf (GAP-104a-2: new bind password; GAP-104a-4: ssh service + sshPublicKey).
	// Per-node hostname is looked up so the sssd.conf comment is accurate; fallback to nodeID.
	nodeHostname := nodeID
	...
	log.Info().Str("node_id", nodeID).Msg("ldap fanout: sssd.conf applied")
	m.audit.Record(ctx, "", "clustr-serverd", db.AuditActionLDAPBindPasswordApplied,
		"node", nodeID, "",
		nil,
		map[string]string{"service_bind_dn": row.ServiceBindDN},
	)

	// 3. Push sshd AuthorizedKeysCommand drop-in (GAP-104a-4).
	// This wires SSH pubkey auth via LDAP (sshPublicKey attribute) on already-deployed nodes.
	// Non-fatal: a missing sssd-client RPM won't have sss_ssh_authorizedkeys, but writing
	// the drop-in is harmless — sshd logs a warning and falls back to authorized_keys files.
	if err := m.sendConfigPush(ctx, hub, nodeID, "sshd-ldap-keys", sshdLDAPKeyDropin); err != nil {
		log.Warn().Err(err).Str("node_id", nodeID).Msg("ldap fanout: sshd-ldap-keys push failed (non-fatal)")
		// Non-fatal: sssd.conf and CA cert are the critical pushes. Log the error but
		// return OK=true so a missing sshd-ldap-keys doesn't block the fanout report.
	} else {
		log.Info().Str("node_id", nodeID).Msg("ldap fanout: sshd-ldap-keys applied")
		m.audit.Record(ctx, "", "clustr-serverd", db.AuditActionLDAPSSHDKeysApplied,
			"node", nodeID, "",
			nil,
			map[string]string{"dropin": "50-clustr-ldap-keys.conf"},
		)
	}

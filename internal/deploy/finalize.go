...
func runGrub2InstallEFIInChroot(ctx context.Context, mountRoot string) error {
	...
	// FIX-EFI (#225): on UEFI hosts, ensure a PXE entry leads BootOrder so
	// the next reimage doesn't get blocked by an OS entry left over from a
	// prior life of the disk.  Best-effort: a failure here NEVER fails an
	// otherwise successful deploy — the deployed node will boot via the
	// removable-media path (\EFI\BOOT\BOOTX64.EFI) regardless of NVRAM
	// order, and the worst case is that the operator has to power-cycle
	// once for PXE to win on next reboot.  On BIOS systems this is a true
	// no-op (RepairBootOrderForReimage exits early when /sys/firmware/efi
	// is absent).
...
func writeLDAPConfig(ctx context.Context, mountRoot string, ldapCfg *api.LDAPNodeConfig) error {
	...
	// ── #112: AuthorizedKeysCommand for SSH pubkey via LDAP ───────────────────
	// Sprint 15 embedded openssh-lpk schema (sshPublicKey attribute). Wire sshd
	// to use sss_ssh_authorizedkeys so SSH logins work via LDAP-stored public keys.
	// Written as a drop-in so it survives base-image sshd_config resets.
	sshdDropinDir := filepath.Join(mountRoot, "etc", "ssh", "sshd_config.d")
	if err := os.MkdirAll(sshdDropinDir, 0o755); err != nil {
		log.Warn().Err(err).Str("path", sshdDropinDir).
			Msg("finalize: mkdir sshd_config.d failed (non-fatal)")
	} else {
		const sshdDropin = "# 50-clustr-ldap-keys.conf — generated by clustr-serverd\n" +
			"# Enables SSH public-key authentication via LDAP (sshPublicKey attribute).\n" +
			"# DO NOT EDIT — managed by clustr.\n" +
			"AuthorizedKeysCommand /usr/bin/sss_ssh_authorizedkeys\n" +
			"AuthorizedKeysCommandUser nobody\n"
		dropinPath := filepath.Join(sshdDropinDir, "50-clustr-ldap-keys.conf")
		if err := os.WriteFile(dropinPath, []byte(sshdDropin), 0o644); err != nil {
			log.Warn().Err(err).Str("path", dropinPath).
				Msg("finalize: write sshd AuthorizedKeysCommand drop-in failed (non-fatal)")
		} else {
			log.Info().Str("path", dropinPath).
				Msg("finalize: sshd AuthorizedKeysCommand drop-in written (LDAP pubkey auth enabled)")
		}
	}

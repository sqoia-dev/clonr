...
func (h *RacksHandler) ListRacks(w http.ResponseWriter, r *http.Request) {
	...
	if strings.Contains(includeParam, "positions") {
		for i := range racks {
	...
	if strings.Contains(includeParam, "enclosures") {
		for i := range racks {
			encs, eErr := h.DB.ListEnclosuresByRack(r.Context(), racks[i].ID)
			if eErr != nil {
				log.Error().Err(eErr).Str("rack_id", racks[i].ID).Msg("racks: list enclosures")
				writeError(w, eErr)
				return
			}
			if encs == nil {
...
// GetRack fetches a single rack with its positions and enclosures embedded.
func (h *RacksHandler) GetRack(w http.ResponseWriter, r *http.Request) {
	...
	encs, err := h.DB.ListEnclosuresByRack(r.Context(), id)
	if err != nil {
		log.Error().Err(err).Str("rack_id", id).Msg("racks: list enclosures")
		writeError(w, err)
		return
	}
	if encs == nil {

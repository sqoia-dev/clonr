...
fetch_lsblk_from_server() {
    ...
    NEEDED_LIBS=$(remote_exec "ldd /usr/bin/lsblk 2>/dev/null" | \
        grep -oE '/[^ ]+\.so[^ ]*' | sort -u 2>/dev/null || true)
...
if [[ "$LSBLK_INSTALLED" == "true" ]]; then
    ...
    echo "               Fetch lsblk from the server host and re-run the build." >&2
fi
...
${transitive}"
    ...
    #
    # Resolution order:
    #   1. CI vendored build: ${SCRIPT_DIR}/../build/udpcast/dist/amd64/udp-receiver
    #      (MODULES_PATH set + file present — CI only)
    #   2. RPM-shipped binary: /usr/share/clustr/vendor/udpcast/udp-receiver
    #      (clustr-serverd RPM installs this; hit on autodeploy/production hosts)
    #   3. SSH pull from server host PATH: legacy dev fallback
    #   4. Disabled — build still succeeds, node falls back to unicast HTTP fetch.
    echo "[+] Installing udp-receiver (UDPCast multicast)..."
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    VENDORED_UDPRECV="${SCRIPT_DIR}/../build/udpcast/dist/amd64/udp-receiver"
    RPM_UDPRECV="/usr/share/clustr/vendor/udpcast/udp-receiver"
    if [[ -n "${MODULES_PATH:-}" && -f "$VENDORED_UDPRECV" && -x "$VENDORED_UDPRECV" ]]; then
        # 1. CI mode: use vendored binary built from the same source as udp-sender
        mkdir -p "$WORKDIR/usr/bin"
    ...
    elif [[ -f "$RPM_UDPRECV" && -x "$RPM_UDPRECV" ]]; then
        # 2. RPM-shipped binary — present on hosts running the clustr-serverd RPM
        mkdir -p "$WORKDIR/usr/bin"
        if cp -f "$RPM_UDPRECV" "$WORKDIR/usr/bin/udp-receiver" && chmod 755 "$WORKDIR/usr/bin/udp-receiver"; then
            UDPRECV_SIZE=$(du -sh "$RPM_UDPRECV" 2>/dev/null | cut -f1 || echo "?")
            echo "  [+] udp-receiver installed from RPM-shipped binary (${UDPRECV_SIZE})"
        else
            echo "  [!] RPM-shipped udp-receiver copy failed — multicast disabled" >&2
        fi
    else
        # 3. Dev/remote mode: look up on the server host (legacy path)
        ...
        if [[ -n "$UDPRECV_PATH" ]]; then
            ...
            echo "       On RPM host: ${RPM_UDPRECV} was not found" >&2
        fi
    fi
fi
...
echo "Initramfs ready at $OUTPUT ($SIZE)"

...
//lint:ignore U1000 used by the auto-hostname regeneration endpoint (NODES-REGEN-HOSTNAME) planned in Sprint 39
...
func (h *NodesHandler) PatchNode(w http.ResponseWriter, r *http.Request) {
	...
	if v, ok := patch["base_image_id"]; ok {
		var s string
		_ = json.Unmarshal(v, &s)
		// Reject initramfs-role images at assignment time — they are boot-time
		// infrastructure and must never be set as a node's deployable target image.
		if s != "" {
			img, imgErr := h.DB.GetBaseImage(r.Context(), s)
			if imgErr != nil {
				writeError(w, imgErr)
				return
			}

...
// TestConfigApplier_WritesSSHDLDAPKeys verifies that the sshd-ldap-keys target
// (GAP-104a-4) writes the drop-in to
// <rootDir>/etc/ssh/sshd_config.d/50-clustr-ldap-keys.conf with mode 0644.
// The parent sshd_config.d/ directory must be created automatically since it may
// not exist in the node image.
func TestConfigApplier_WritesSSHDLDAPKeys(t *testing.T) {
	root := t.TempDir()
	// Intentionally do NOT pre-create /etc/ssh/sshd_config.d — ApplyOne must create it.

	content := "# 50-clustr-ldap-keys.conf — generated by clustr-serverd\n" +
		"AuthorizedKeysCommand /usr/bin/sss_ssh_authorizedkeys\n" +
		"AuthorizedKeysCommandUser nobody\n"
	payload := ConfigPushPayload{
		Target:   "sshd-ldap-keys",
		Content:  content,
		Checksum: checksum(content),
	}
	...
	dropinPath := filepath.Join(root, "etc", "ssh", "sshd_config.d", "50-clustr-ldap-keys.conf")
	got, err := os.ReadFile(dropinPath)
	if err != nil {
		t.Fatalf("read sshd drop-in: %v", err)
	}
	if string(got) != content {
		t.Errorf("sshd drop-in content = %q, want %q", string(got), content)
	}

	info, err := os.Stat(dropinPath)
	if err != nil {
		t.Fatalf("stat sshd drop-in: %v", err)
	}
	if perm := info.Mode().Perm(); perm != 0o644 {
		t.Errorf("sshd drop-in mode = %04o, want 0644", perm)
	}
}

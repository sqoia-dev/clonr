...
func (c *Client) dispatchServerMessage(msg ServerMessage) {
	...
	case "operator_exec_request":
		// Dispatched in a goroutine so long-running commands (>readTimeout) do not
		// block the readLoop. A blocked readLoop cannot reset the read deadline, which
		// causes the connection to die mid-execution when the deadline fires (#166).
		go c.handleOperatorExecRequest(msg)

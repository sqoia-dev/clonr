...
func (m *Manager) restoreFromDB(ctx context.Context) {
	...
	// Auto-generate the per-cluster repo GPG signing key if the module is enabled
	// but no key has been generated yet.  This covers two cases:
	//   1. Fresh install: Enable() generates the key; this is a no-op.
	//   2. Existing install that was enabled before auto-keygen landed: the key
	//      is missing and restoreFromDB generates it on first startup after upgrade.
	// Idempotent: InitRepoGPGKey checks for an existing key before generating.
	// Non-fatal: a missing key means RPM signing is skipped at build time (not a
	// crash), and the operator can trigger POST /api/v1/slurm/repo/init-gpg manually.
	if row.Enabled {
		if _, err := m.db.SlurmGetRepoGPGConfig(ctx); err != nil {
			if genErr := m.InitRepoGPGKey(ctx); genErr != nil {
				log.Warn().Err(genErr).Msg("slurm: restoreFromDB: auto-generate repo GPG key failed (non-fatal) — run POST /slurm/repo/init-gpg manually")
			} else {
				log.Info().Msg("slurm: repo GPG key auto-generated on startup (module was enabled before auto-keygen)")
			}
		}
	}
}

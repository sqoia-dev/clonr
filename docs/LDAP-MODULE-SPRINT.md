# clonr LDAP Module — Sprint Plan (v1)
...
Add an optional "LDAP" module to clonr. When an operator enables it in the webui, clonr:

1. Stands up a self-hosted OpenLDAP (slapd) instance on the clonr-serverd host, configured end-to-end following best practices.
...
The feature is opt-in. A clonr install with the module disabled must behave identically to today.
...
| **CA cert** | RSA 4096, **30-year validity**, stored at `/etc/clonr/pki/ca.crt`, key at `/etc/clonr/pki/ca.key` (mode 0600) | Rotating a root of trust across every compute node is painful; avoid doing it often. LDAP clients don't enforce the 825-day browser rule. |
| **Server cert** | RSA 4096, **5-year validity**, auto-renewed at year 4 by a background goroutine. SAN must include hostname, IP, and `clonr.local`. | Long enough that a neglected cluster doesn't break; short enough that rotation is survivable. Missing IP SAN is the #1 sssd TLS failure. |
...
type LDAPNodeConfig struct {
    ServerURI         string // "ldaps://clonr.local:636"
    BaseDN            string
    ...
    CACertPEM         string // written to /etc/pki/ca-trust/source/anchors/clonr-ca.crt and /etc/openldap/certs/
}

// internal/ldap/manager.go — not wire-exposed
type Manager struct {
    db      *sql.DB
    cfgDir  string  // /etc/clonr/ldap
    dataDir string  // /var/lib/clonr/ldap
    pkiDir  string  // /etc/clonr/pki
    // ...
}
...
| POST | `/backup` | Run `slapcat -n 1` to a timestamped LDIF under `/var/lib/clonr/ldap/backups/`, return filename |

Webui pages (static + JS under `clonr-static/`):
...
clonr-static/pages/ldap.html      # (or equivalent per existing static layout)
clonr-static/pages/ldap-users.html
clonr-static/pages/ldap-groups.html
clonr-static/js/ldap.js

deploy/systemd/clonr-slapd.service
deploy/polkit/50-clonr-slapd.rules
deploy/install/ldap-bootstrap.sh  # masks distro slapd.service, installs our unit, creates /var/lib/clonr/ldap, polkit file
```
...
Each phase commits cleanly and pushes. CI runs the build; `clonr-autodeploy.timer` on the clonr VM rebuilds and hot-restarts within 2 minutes.
...
- `slapd.go`: render seed LDIF from template, run `slapadd -n 0 -F <config>`, `systemctl start clonr-slapd`.
...
- Config-drift warning (hash of `/etc/clonr/ldap/slapd.d/` compared to last-seeded hash).
...
3. `clonr-autodeploy.timer` on `192.168.1.151` polls `origin/main` every 2 minutes, rebuilds binaries and initramfs, hot-restarts `clonr-serverd`. Watch with:
   ```
   ssh -i ~/.ssh/id_ed25519 root@192.168.1.151 "journalctl -u clonr-autodeploy.service -f"
   ```
4. To test uncommitted changes on the VM: `systemctl stop clonr-autodeploy.timer` on the VM, rsync your tree, build there. Resume with `systemctl start clonr-autodeploy.timer`.
...
- [ ] Fresh clonr install with module disabled behaves identically to today.

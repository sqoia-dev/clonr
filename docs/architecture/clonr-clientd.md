...
`clonr-clientd` is a persistent, lightweight daemon installed on every deployed
node. It maintains a single multiplexed WebSocket connection back to
`clonr-serverd`, enabling config push, remote diagnostics, log streaming, and
...
- A token already written to `/etc/clonr/node-token` during finalization (`internal/deploy/phonehome.go`)
- A `verify-boot-url` already written to `/etc/clonr/verify-boot-url`
...
clientd boots
    |
    v
Read /etc/clonr/node-token
Read /etc/clonr/clonrd-url (new file, written during finalization)
    |
    v
...
`clonr-clientd` uses the same node-scoped API key already written to
`/etc/clonr/node-token` during finalization. No new auth mechanism needed.
...
written to the new rootfs during finalization. `clonr-clientd` reads the token
...
What the server can push to a node via `clonr-clientd`:

| Operation | Allowed | Rationale |
|---|---|---|
| Write files in `/etc/clonr/` managed namespace | Yes | Scoped to clonr-managed configs only |
...
| Write files outside `/etc/clonr/`, `/etc/slurm/`, or other config targets | No | Filesystem scope enforced in client |
...
| Token theft from `/etc/clonr/node-token` | Attacker can impersonate the node | 0600 permissions (already set by phonehome.go); token is node-scoped, cannot escalate to admin |
| MitM on the WebSocket connection | Can intercept logs, replay config pushes | TLS required (CLONR_TLS or Caddy termination); clientd refuses plaintext ws:// in production mode |

**Key architectural constraint:** `clonr-clientd` runs as a dedicated
`clonr-clientd` system user (not root) with a writable namespace limited to
`/etc/clonr/`, `/etc/sssd/`, `/etc/slurm/`, `/etc/hosts`, `/etc/ntp.conf`, and
`/etc/resolv.conf`. File writes outside these paths are rejected by the daemon
before attempting them, regardless of server instruction. The `clonr-clientd`
...
**Heartbeat interval:** 60 seconds. Configurable via `/etc/clonr/clientd.conf`.
...
A new standalone binary: `clonr-clientd`. It does NOT share a binary with the
existing `clonr` CLI. Rationale:

- The `clonr` CLI is a deploy-time tool (initramfs); linking it with a
  persistent daemon's dependencies creates unnecessary coupling.
- `clonr-clientd` needs to be a small, static binary for easy image baking.
- Separate binary = separate systemd unit = separate resource limits = simpler
  code ownership.

**Module path:** `cmd/clonr-clientd/main.go`, package
`github.com/sqoia-dev/clonr/cmd/clonr-clientd`.
...
1. Writes `/etc/clonr/clonrd-url` (the WebSocket endpoint URL)
2. Writes `/etc/clonr/clientd.conf` (heartbeat interval, service whitelist)
3. Writes `/etc/systemd/system/clonr-clientd.service` from embedded unit
4. Copies the `clonr-clientd` binary to `/usr/local/bin/clonr-clientd`
5. Creates the `WantedBy=multi-user.target` symlink (same pattern as `injectPhoneHome`)

The `clonr-clientd` binary is embedded in the server binary at build time (same
pattern as `internal/bootassets/assets.go` for `ipxe.efi`). The server's
`CLONR_CLIENTD_BIN_PATH` env var allows overriding with a local path for
development.

**Files written to deployed rootfs:**

```
/etc/clonr/node-token          (existing, reused)
/etc/clonr/clonrd-url          (new: ws://clonr-server:8080/api/v1/nodes/{id}/clientd/ws)
/etc/clonr/clientd.conf        (new: interval, service whitelist, log filters)
/usr/local/bin/clonr-clientd   (new: the daemon binary)
/etc/systemd/system/clonr-clientd.service  (new: systemd unit)
```
...
```ini
[Unit]
Description=clonr node management daemon
...
ReadWritePaths=/etc/clonr /etc/sssd /etc/hosts /etc/chrony.conf /etc/ntp.conf /etc/resolv.conf
ProtectHome=true
PrivateTmp=true
...
The `clonr-clientd` system user is created by the finalization step (using the
existing sysaccounts infrastructure — `internal/deploy/sysaccounts.go`).

### Token Management

1. At startup, `clonr-clientd` reads `/etc/clonr/node-token` into memory.
...
5. The token file is never written by `clonr-clientd` — only by the
   finalization layer.
...
Incoming `log_batch` messages from `clonr-clientd` are processed through the
...
Node boots
    |
    clonr-clientd.service starts (After=network-online.target)
    |
    Reads /etc/clonr/node-token, /etc/clonr/clonrd-url
    |
    GET ws://clonr-server:8080/api/v1/nodes/{id}/clientd/ws
         Authorization: Bearer <node-token>
    |
...
2. `cmd/clonr-clientd/main.go`: systemd notify integration, signal handling,
   token file reading
...
9. `internal/deploy/phonehome.go`: extend `injectPhoneHome` → extract into `injectNodeAgents`,
   add `clonrd-url` and `clientd.conf` file writes
10. Build embedding: `internal/bootassets/clientd.go` (`//go:embed clonr-clientd`)
...
existing `Makefile` gets a `clonr-clientd` target.
...
| File | Purpose |
|---|---|
| `cmd/clonr-clientd/main.go` | Daemon entry point |
...
| `deploy/systemd/clonr-clientd.service` | Template for node systemd unit (embedded) |
| `deploy/systemd/clonr-clientd-node.service` | Embedded in finalization layer |
...
| `internal/deploy/finalize.go` | Add role-aware `writeSlurmConfig()` step; install Slurm binary artifact from `slurm_builds` when active build is set; write munge.key from `slurm_secrets`; write and enable role-specific services; add `/etc/sudoers.d/clonr-clientd-slurm`; add `/etc/slurm/` and `/etc/munge/` to `ReadWritePaths` |
...
| `deploy/systemd/clonr-clientd.service` | Add `/etc/slurm/` and `/etc/munge/` to `ReadWritePaths` |
| `Makefile` | Add `clonr-clientd` build target |
...
Slurm configuration management is implemented as a **first-class clonr module**,
...
on the clonr server and performs DIT operations (user/group CRUD). The Slurm
...
lifecycle (Enable/Disable) controls whether clonr treats itself as the
...
// nodes — it only stops clonr from acting as the authoritative config source.
...
CREATE TABLE IF NOT EXISTS slurm_builds (
    ...
    artifact_path    TEXT,                  -- relative path within CLONR_DATA_DIR
...
-- AES-256-GCM encrypted with CLONR_SECRET_KEY. Never returned in plaintext via API.
CREATE TABLE IF NOT EXISTS slurm_secrets (
    key_type         TEXT PRIMARY KEY,      -- "munge_key"
    encrypted_value  TEXT NOT NULL,         -- base64(AES-256-GCM(plaintext, CLONR_SECRET_KEY))
...
clonr manages the following Slurm config files. These are the files where
...
**Files clonr does NOT manage:**

- `/etc/slurm/epilog`, `/etc/slurm/prolog`, `/etc/slurm/task_prolog` — scripts,
  not config files. They belong in the clonr image, not pushed at runtime.
- Any site-local config referenced via `Include` directives — clonr does not
...
**Permission for `systemctl restart slurmd`:** The `clonr-clientd` system user
requires a narrow sudoers entry, written by `writeSlurmConfig()` in finalize.go:

```
# /etc/sudoers.d/clonr-clientd-slurm
clonr-clientd ALL=(ALL) NOPASSWD: /bin/systemctl restart slurmd
clonr-clientd ALL=(ALL) NOPASSWD: /bin/systemctl restart slurmctld
```
...
writeSlurmConfig(nodeID, slurmConfig, rootfsMountPath):
    ...
    write /etc/sudoers.d/clonr-clientd-slurm (sudoers entry for slurmd restart)
...
The Slurm module has a dedicated nav section in the clonr UI, appearing alongside
...
1. **Admin scope only.** All Slurm config write and push endpoints require
   admin-scoped API keys. Node-scoped keys (used by `clonr-clientd`) can only
...
3. **Node-side path enforcement.** `clonr-clientd` only accepts writes to the
...
4. **`slurmdbd.conf` encrypted at rest.** Stored as `AES-256-GCM(content,
   CLONR_SECRET_KEY)` in `slurm_config_files.content`. Decrypted in memory only
...
5. **Worst-case blast radius is bounded to the cluster.** A compromised server
   ...
   to escape to arbitrary file writes or root execution. The `clonr-clientd` user
...
- **Module-scoped singleton design (`slurm_module_config` id=1).** This is
  the correct model: clonr is either the authoritative Slurm config source for
  the cluster or it is not. A per-cluster multi-tenant model would be the
  correct next step if clonr ever manages multiple independent Slurm clusters,
  but that requires a schema migration. For v1 (one cluster per clonr instance),
...
the clonr data store, and distributing them during imaging is the correct model.
...
module's config push path. Build artifacts are stored in the clonr data store
...
Admin triggers build via UI or API
    ...
    3. Run configure:
       ./configure \
         ...
         --with-munge \
         --with-pmix=/opt/clonr/deps/pmix/{version} \
         --with-hwloc=/opt/clonr/deps/hwloc/{version} \
         --with-ucx=/opt/clonr/deps/ucx/{version} \
         --enable-pam \
         --with-lz4 \
         --with-json \
         --with-http-parser \
    ...
    |
    6. Store artifact:
       Write tarball to CLONR_DATA_DIR/slurm-builds/{version}/{arch}/slurm-{version}.tar.gz
...
- All build operations run as a dedicated `clonr-builder` system user with write
  access only to `CLONR_DATA_DIR/slurm-builds/` and `/tmp/`. No root.
- The `make install DESTDIR` step does not touch the live system. Installation
  into the running clonr server host is never performed.
...
4. Downgrading across a minor release boundary is not supported by Slurm and
   must be blocked by clonr (rollback within the same minor is safe).

clonr enforces rules 1, 2, and 4 automatically. Rule 3 is validated and surfaced
...
[Phase 3: Compute nodes — rolling batches]
    For each batch of N nodes (admin-configured batch_size, default 10):
        Drain batch: scontrol update NodeName=node01,node02,...,nodeN State=DRAIN Reason="clonr-upgrade"
...
type SlurmBinaryPushPayload struct {
    ...
    ArtifactURL    string `json:"artifact_url"`   // internal HTTP endpoint on clonr server
    ArtifactSHA256 string `json:"artifact_sha256"`
}
```

The node fetches the tarball via HTTP from the clonr server (not pushed inline
...
clonr server host) to avoid contaminating the host system.
...
maintained by the clonr maintainers and updated with each Slurm release. The
...
clonr manages the munge key as a cluster-level secret:
...
munge.key lifecycle:
    ...
    Store encrypted: slurm_secrets table, key_type="munge_key",
                     value=AES-256-GCM(key, CLONR_SECRET_KEY)
    |
    [during finalization]
...
Dependencies are stored in the same `CLONR_DATA_DIR/slurm-builds/` tree:
...
clonr manages these scripts with the same versioning model as config files
...
The destination path for each script is a site-local choice. clonr stores the
...
clonr would either push all configs and scripts to all nodes (incorrect and

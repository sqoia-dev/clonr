...
func newAlertsCmd() *cobra.Command {
	var (
		...
		flagRule     string
		flagJSON     bool
		// stub flags — not yet implemented; silencing is available via the web UI
		flagAck     bool
		flagSilence bool
	)
...
Acknowledgement and silence operations are not yet available via the CLI; use the web UI Alerts tab to silence an alert.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			// Stub flags — inform the user and exit cleanly.
			if flagAck {
				fmt.Fprintln(os.Stderr, "not yet implemented; use the web UI Alerts tab to acknowledge alerts")
				return nil
			}
			if flagSilence {
				fmt.Fprintln(os.Stderr, "not yet implemented; use the web UI Alerts tab to silence alerts")
				return nil
			}
	...
	cmd.Flags().BoolVar(&flagAck, "ack", false, "Acknowledge an alert (not yet implemented; use the web UI)")
	cmd.Flags().BoolVar(&flagSilence, "silence", false, "Silence an alert (not yet implemented; use the web UI)")

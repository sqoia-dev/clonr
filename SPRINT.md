...
## Sprint 25 lab matrix re-run results (2026-05-02)

### Phase 1 — VMID 299 state assessment

Finding: **State B** — VM existed (created 2026-05-01), install complete, but NOT yet converted to template.

- Boot order was still `order=ide2;ide3` pointing at `Rocky-9.7-autoinstall.iso`
- Disk (`local-lvm:vm-299-disk-0`, 32G, 6.81% used ≈ 2.18 GB) had a valid MBR partition table: 1M biosboot (p1), 1G /boot (p2), 31G / (p3)
- Verified via loopback mount: `Rocky Linux release 9.7 (Blue Onyx)` on p3
- Root cause of no-network on first start: kickstart appended `net.ifnames=0 biosdevname=0` to kernel cmdline but NetworkManager connection was written as `ens18.nmconnection` (installer-env name). Fixed offline by replacing with `eth0.nmconnection` on both test clones.
- sshd_config: kickstart `%post` sed did not persist into the installed system (lines remained `#PermitRootLogin prohibit-password`, `#PasswordAuthentication yes`). Fixed offline on both test clones before validation.

### Phase 2 — Template finalization

Steps taken:
1. Fixed boot order: `qm set 299 --boot order=scsi0 --ide2 none,media=cdrom`
2. Confirmed boot from disk via QEMU monitor (`-boot strict=on`, `scsi0 bootindex=100`, no cdrom in chain)
3. Converted: `qm template 299` — renamed LV to `base-299-disk-0`, `template: 1` set
4. `qm list | grep 299` → `rocky9-minimal-template stopped 4096 32.00 0` with template flag

Template is ready. **Note for future use:** before cloning, apply these two offline fixes to the template disk:
- Replace `/etc/NetworkManager/system-connections/ens18.nmconnection` with `eth0.nmconnection` (interface naming mismatch due to `net.ifnames=0` in bootloader args)
- Uncomment `PermitRootLogin yes` and `PasswordAuthentication yes` in `/etc/ssh/sshd_config`

Thin pool overcommit warning surfaced during clone of VMs 301+302: `Sum of all thin volume sizes (356.02 GiB) exceeds thin pool + free space (16.00 GiB)`. Lab data usage is ~2.18 GB per VM so no immediate risk, but the pool needs cleanup before additional clone-heavy work.

### Phase 3 — LDAP/SSSD matrix re-runs

Test environment: VMID 302 (`clustr-test-compute`, 192.168.1.156), fresh clone of template 299. clustr-serverd `v0.2.0-619-g54b33d9` (HEAD `54b33d9`) deployed via binary copy. Bootstrap-admin ran, `clustr/clustr` default creds confirmed working.

| Row | Expected behavior | Actual behavior | Result |
|-----|-------------------|-----------------|--------|
| 1 | Clean slapd bring-up | POST `/api/v1/ldap/internal/enable` → 202 Accepted. Steps logged: EPEL install → openldap-servers dnf → clustr-slapd.service unit write → TLS cert gen → cn=config seed via slapadd → slapd start → DIT seed (base DN + OUs + node-reader + clustr-admins) → DB save. Final status: `enabled=true, running=true, systemd_active=active, port=636`. Elapsed: ~30s. | **PASS** |
| 2 | CA rotation push to enrolled nodes (#109) | No nodes enrolled — VM 302 has no compute nodes registered. | **BLOCKED** — needs at least one enrolled node (second VM running clustr-clientd) |
| 3 | Service-bind password drift detection + push (#110) | After enable: `bind_password_set: true`, `write_capable: true`, `write_status: dm_fallback`. Drift push path not exercisable without enrolled nodes. | **BLOCKED** — needs enrolled node for fanout path |
| 4 | SSSD ldap_uri uses IP fallback (#111) | Source confirmed at `internal/ldap/manager.go:983-991`: `detectPrimaryIP()` builds `ldaps://<IP>:636`, falls back to hostname. Template `sssd.conf.tmpl:24` renders `ldap_uri = {{.ServerURI}}`. End-to-end push not testable without enrolled node. | **PASS (impl confirmed)** — live push blocked on enrolled node |
| 6 | SSH AuthorizedKeysCommand for LDAP pubkey auth (#112) | No enrolled node with SSSD to verify AuthorizedKeysCommand execution path. | **BLOCKED** — needs enrolled node with sssd deployed |

### Verified (Row 1 detail)

Key log lines confirming GAP-S18-1 and GAP-S18-2:

```
ldap: data DIT seeded (base DN + OUs + node-reader account + clustr-admins group)
ldap: module enabled and ready  base_dn=dc=cluster,dc=local
```

API response after enable:
```json
{"groups": [{"cn": "clustr-admins", "gid_number": 10000, "member_uids": [], "description": "Members of this group have full sudo on cluster nodes"}]}
```

- **GAP-S18-1** (sudoers group seeded at enable): PASS — `clustr-admins` seeded automatically with correct description
- **GAP-S18-2** (group named `clustr-admins` not `clonr-admins`): PASS — confirmed `cn=clustr-admins`
- **Service bind DN**: `cn=node-reader,ou=services,dc=cluster,dc=local` — correct
- **CA fingerprint**: present (`876dec3f...`) — TLS cert generated and stored

### New tasks queued

None from FAILs. Three BLOCKED rows need a second VM with clustr-clientd enrolled to unblock:

- **LAB-BLK-1**: Rows 2, 3, 6 blocked on enrolled compute node. Clone VMID 303 from template 299 and enroll against VM 302's clustr-serverd to unblock. Straightforward but requires clientd RPM + provisioning flow.
- **LAB-BLK-2**: Template 299 has two known issues (interface naming, sshd_config) that affect every clone. Update the template disk offline before next wave of clones to eliminate per-VM fixup overhead.

### Out of scope (confirmed)

- **#124 Mellanox + LSI 9361-8i hardware validation**: No such hardware in lab. Still blocked on hardware. Not schedulable until hardware is acquired.

### Infrastructure note

Proxmox thin pool overcommit (pve/data): 356 GiB allocated vs 16 GiB free. Safe for current workload (~2.18 GB actual per VM) but will become a blocker if more full clones are created. Clean up stopped test VMs (201, 202, 203, 206) when lab work is done to reclaim pool space.

---

## Wave 8 — Webapp E2E Audit (2026-05-01, HEAD f26750d → commit f26750d)

### Routes covered (13 of 13)

- `/` (redirect to /nodes) — PASS
- `/login` — PASS
- `/setup` — PASS
- `/set-password` — PASS
- `/nodes` — FIXED-INLINE (see below)
- `/images` — PASS
- `/slurm` — PASS
- `/alerts` — PASS
- `/datacenter` — PASS
...
- `/identity` — PASS
- `/settings` — PASS
- AppShell (nav, connection banner, pending-changes drawer, command palette) — PASS

### Per-route findings

**`/nodes`**
- FIXED-INLINE-f26750d: `NodesPage` query didn't destructure `isError` — on API failure it silently fell through to `EmptyState`, showing "No nodes registered yet" instead of an error message. Added `isError` branch rendering "Failed to load nodes. Reload to retry."

**`/images`**
- PASS: Three tabs (Base Images, Bundles, Initramfs) all have loading/empty/error states. AddImageSheet (URL, TUS upload, server filesystem) all wired. BuildInitramfsSheet live SSE log works. ImageSheet (shell, delete, install instructions) complete.
- PASS: UX-4 `useEventSubscription("images", ...)` wired for live list updates.
- PASS: BUG-5 `{images: [...]}` wrapper is correct — ListImagesResponse shape matches.

**`/slurm`**
- PASS: Overview/Jobs/Partitions tabs all render. Configs, Roles, Scripts, Builds, Upgrades sections present.
- NOTE: No SSE wiring for slurm — polling at 15-30s intervals is intentional (no slurm SSE topic on server).

**`/alerts`**
- PASS: Active/Silenced/History tabs. SilenceButton dropdown wired. RuleModal YAML editor is writable (UX-9 resolved).

**`/datacenter`**
- PASS: Multi-rack tile layout, cross-rack drag (UX-10b confirmed). Unassigned sidebar loads correctly (BUG-2 fixed).
- PASS: BUG-6 — RU height-U selector (1U/2U/4U/8U buttons) visible in `UnassignedNodeRow`, stops drag propagation correctly.
- PASS: AddRack modal, bulk power modal with typed-name confirm.

**`/activity`**
- PASS: Table with select-all checkboxes, delete-selected + clear-filtered flows. Correctly uses polling (no audit SSE topic).

**`/identity`**
- PASS: LDAP Users, Groups, System accounts, LDAP config sections all present.

**`/settings`**
- PASS: APIKeys, LocalUsers, ServerConfig, GPGKeys, TwoStageCommit, BootMenu sections. All CRUD flows wired.

**AppShell elements**
- PASS: UX-1 — live-updates banner only shown after 30s non-open (POL-6 timer, not on every nav).
- PASS: UX-3 — SSE reconnect with jittered backoff (`sseReconnectDelay`).
- PASS: UX-4 — single `ConnectionProvider` owns one EventSource. `useEventInvalidation` wired in nodes + `useEventSubscription` wired in images + groups.
- PASS: RISK-1 — `ShellMutationWarningModal` renders before opening image shell (sessionStorage persistence, not re-prompted within same tab).
- PASS: Pending Changes badge + drawer (commit-all, commit-one, clear-all, clear-one) fully wired.
- PASS: CommandPalette — all navigation shortcuts and deep-link params verified.

### Test suite

- `pnpm test --run`: **19/19 files, 294/294 tests** — clean pass before and after fix.

### Surprises

- `ImageShell.tsx` imports `@xterm/xterm` dynamically to avoid SSR, but `node-detail-tabs.tsx` imports it statically — Vite warns about the ineffective dynamic import at build time. No runtime impact. Queued as UX-11.

...
export type ImageStatus = "building" | "ready" | "error" | "archived" | "interrupted" | "corrupt" | "blob_missing"

// ReconcileOutcome mirrors pkg/reconcile.Outcome on the server.
export type ReconcileOutcome = "ok" | "healed" | "quarantined" | "blob_missing" | "no_change" | "re_finalized"
...
export interface Bundle {
  id: string             // slurm_builds UUID
  name: string           // e.g. "slurm-25.11.5-x86_64"
  slurm_version: string  // e.g. "25.11.5"
  bundle_version: string // same as slurm_version for build entries
  sha256: string         // artifact tarball SHA256 hex
  kind: string           // "build"
  source: string         // "clustr-build-pipeline"
  status: string         // "building" | "completed" | "failed"
  is_active: boolean     // true when this is the active RPM source
  nodes_using: number    // count of enrolled nodes running this build
  last_deployed_at?: number  // unix timestamp of most recent deploy
  sig_status?: string    // "signed" | "unsigned" | "unknown"
  started_at?: number
  completed_at?: number
}

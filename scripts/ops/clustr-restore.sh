#!/usr/bin/env bash
# clustr-restore.sh — restore clustr SQLite database from a named backup file
#
# Usage:
#   sudo ./clustr-restore.sh <backup-file>
#
# Examples:
#   sudo ./clustr-restore.sh /var/lib/clustr/backups/clustr-20260413-020001.db
#   sudo ./clustr-restore.sh clustr-20260413-020001.db   # looked up in BACKUP_DIR
#
# The script stops clustr-serverd, validates the backup, replaces the live DB,
# then restarts clustr-serverd. It leaves the old DB as .pre-restore.<timestamp>
...
# ---------------------------------------------------------------------------
# Confirmation prompt (skipped when CLUSTR_RESTORE_YES=1)
# ---------------------------------------------------------------------------
if [[ "${CLUSTR_RESTORE_YES:-0}" != "1" ]]; then
    echo ""
    ...
    echo "  clustr-serverd will be stopped and restarted."
    echo ""

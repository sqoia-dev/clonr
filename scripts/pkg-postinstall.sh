...
echo ""
echo "  5. (Optional) Enable RPM auto-update — the timer polls pkg.sqoia.dev"
echo "     every 15 minutes and upgrades clustr / clustr-serverd in place."
echo "     Defers automatically while a deploy / image-build / reimage is"
echo "     in flight (#225 RPM-UPDATE-1)."
echo "       systemctl enable --now clustr-rpm-update.timer"
echo ""
echo "  6. (Stateless NFS mode only — skip if not using stateless_nfs nodes)"
echo "     Enable the NFS server so clustr-privhelper can run exportfs -ra:"
echo "       systemctl enable --now nfs-server"
echo "     The clustr-privhelper nfs-export verb writes /etc/exports entries"
echo "     and reloads the NFS daemon; nfs-server.service must be running."
echo ""
